...
/*   Updated: 2022/07/16 07:49:46 by ldurieux         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int	get_board(int **board, char *str)
{
	int	i;
	int	j;
...
int	make_board(int **board)
{
	...
	while (i < 4)
	{
		board[i + 1] = malloc(sizeof(int) * board[0][i / 2]);
		if (!board[i + 1])
		{
			free_array(board, 5);
			return (0);
